...
const colorTextStyles = Object.entries(colors).reduce(
  (accum, [color, code]) => {
    const mapping = {};
...
const colorBackgroundStyles = Object.entries(colors).reduce(
  (accum, [color, code]) => {
    const mapping = {};
    for (let k = 10; k <= 100; k += 10) {
      mapping[`${color}-${k}`] = {
        backgroundColor: `${code} ${k / 100})`,
      };
    }
    return {
      ...accum,
